...
const { md5File, readFile, writeFile, asyncGlob } = require('./utils');
...
async function onPostBuild(pluginOptions) {
  const { uploader, ...rest } = pluginOptions
  try {
    ...
    const uploadedOther = otherFiles.length ? await upload(uploader, otherFiles) : {};
    ...
    const uploadedRewrite = await upload(uploader, [PUBLIC_APP_PATH, PUBLIC_WEBPACK_RUNTIME_PATH]);
...
async function gatherStaticFiles() {
  try {
    return await asyncGlob(`${PUBLIC_PATH}/**/*.?(js|css|json|png)`);
  } catch (err) {
    console.error(err);
    return err;
  }
